...
contract DepositHelper is Ownable {
  ...
  function depositHelper(
    ...
    if (_tokenAddress == LPAddress) {
      ...
      XPH.safeApprove(address(router), uint256(-1));
      calAndSwap();
      ...
      if (XPH.myBalance() > 0) {
        XPH.safeTransfer(msg.sender, XPH.myBalance());
      }
  ...
  function depositValue(uint256 _amount) public view returns (uint256 value_) {
    ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(BUSD, XPH));
    (uint256 token0Reserve, uint256 token1Reserve, ) = lpToken.getReserves();
    (uint256 debtReserve, uint256 relativeReserve) = BUSD == lpToken.token0()
    ...
    if (swapAmt > 0) {
      ...
      (path[0], path[1]) = isReversed ? (XPH, BUSD) : (BUSD, XPH);
      uint[] memory amounts = router.getAmountsOut(swapAmt, path);
      (uint256 amount0, uint256 amount1) = BUSD == lpToken.token0()
